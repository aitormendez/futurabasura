<div id="solapa" class="fixed top-0 z-40 w-screen h-screen py-8 pl-0 pr-8 bg-white sm:pr-16 sm:py-16">
  <ul class="my-4 contents">
    ...
    <li class="">
      <a href="{{ $item->url }}" class="text-sm tracking-widest text-black uppercase sm:text-xl">{{ $item->label }}</a>
    </li>
    @endforeach
  </ul>
  <ul class="my-4 shop">
    ...
    <ul class="flex">
      ...
      <li class="li-cart">
        <a href="/cart" class="flex cart-link nav-item hover:text-allo">
          <div class="flex items-center justify-end flex-grow flex-shrink pl-8 pr-8 bg-white cart sm:items-start sm:pt-8">
            <span>cart</span>
          </div>
